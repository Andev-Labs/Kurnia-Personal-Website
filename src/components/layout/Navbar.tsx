  ...
  return (
    <header className="sticky top-0 z-40 border-b-3 border-ink bg-cream">
      <nav className="page-x flex items-center justify-between gap-5 py-4 md:py-5">
        ...
        <div className="hidden items-center gap-[22px] text-[14.5px] font-semibold md:flex">
          {NAV_LINKS.map((link) => (
        ...
        <button
          ...
          className="grid size-[42px] cursor-pointer place-items-center rounded-xl border-3 border-ink bg-sun shadow-hard-3 md:hidden"
        >
      ...
      <div
        ...
        aria-hidden="true"
        className={cn(
          'fixed inset-0 z-50 bg-ink/60 transition-opacity duration-200 md:hidden',
      ...
      <div
        ...
        inert={!isOpen}
        className={cn(
          'fixed inset-y-0 right-0 z-50 flex w-[min(300px,85vw)] flex-col border-l-3 border-ink bg-cream transition-transform duration-200 md:hidden',
          isOpen ? 'translate-x-0' : 'translate-x-full',
        )}
      >
        <div className="flex items-center justify-between gap-3 border-b-3 border-ink bg-lavender px-[22px] py-4">
          <span className="font-display text-[13px] tracking-[0.04em]">MENU</span>
          <button
            type="button"
            onClick={close}
